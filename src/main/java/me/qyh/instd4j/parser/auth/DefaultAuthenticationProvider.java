...
public class DefaultAuthenticationProvider extends AuthenticationProvider {
    ...
    public DefaultAuthenticationProvider(String username, String password, TwoFactorCodeProvider twoFactorCodeProvider, CookieStorePersistence persistence) {
        super(username);
    ...
    private void twoFactorLogin(HttpClientContext context, String identifier, CloseableHttpClient client) throws IOException {
        ...
        Future<String> future = twoFactorCodeProvider.getCodeFuture(getUsername());
        String code;
        try {
            code = future.get();
        ...
        post.setEntity(entity);
        try {
            HttpUtils.connect(client, post, context);
        } catch (InvalidStateCodeException e) {
            JsonExecutor executor = new JsonExecutor(e.getContent()).execute("error_type");
            if (executor.isPresent()) {
                String errorType = executor.getAsString();

...
//			Debug.dumpBuffer(System.out, "-- mac hash --", hash);
    ...
    class Param implements AlgorithmParameterSpec {
        ...
        public final Param setKey(final byte[] key) {
            ...
            assert key.length <= Spec.max_key_bytes : Engine.Assert.assertFail("key.length", key.length, Engine.Assert.inclusiveUpperBound, Spec.max_key_bytes);
        ...
        public final Param setSalt(final byte[] salt) {
            ...
            assert salt.length <= Spec.max_salt_bytes : Engine.Assert.assertFail("salt.length", salt.length, Engine.Assert.inclusiveUpperBound, Spec.max_salt_bytes);
